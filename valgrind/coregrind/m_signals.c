 ...
 ***** Does not Work with NetBSD , remove this notice once it works ***** 
...
/* Define to give more sanity checking for signals. */ #define DEBUG_SIGNALS
...
/* Maximum usable signal. */ Int VG_(max_signal) = _VKI_NSIG - 1; /* XXX i put teh -1 - hsaliak */
...
void calculate_SKSS_from_SCSS ( SKSS* dst )
{
   ...
   for (sig = 1; sig <= _VKI_NSIG; sig++) {
	 ...
	 if (scss.scss_per_sig[sig].scss_handler == VKI_SIG_DFL)
	   ...
	   skss_handler = async_signalhandler; /* changed to sync_signalhandler - reverted */
	 break;
	 ...
	 if (sig == VG_SIGVGKILL)
	    ...
	    if (scss_handler == VKI_SIG_IGN)
	      ...
	      skss_handler = async_signalhandler; /* all sync_signal handler this changed */
	 }
	 break;
      }
...
#define MYSIG(name)  _MYSIG(name) 
	asm(
		...
		MYSIG(__NR___sigaction_sigtramp)
#else
...
/* True if we are on the alternate signal stack.  */ static Bool on_sig_stack ( ThreadId tid, Addr m_SP )
{
...
/* Block all host signals, dumping the old mask in *saved_mask. */ static void block_all_host_signals ( /* OUT */ vki_sigset_t* saved_mask )
{
   Int           ret;
...
/* Restore the blocking mask using the supplied saved one. */ static void restore_all_host_signals ( /* IN */ vki_sigset_t* saved_mask )
{
   Int ret;
...
/* Hit ourselves with a signal using the default handler */ void VG_(kill_self)(Int sigNo)
{
 ...
 */ static void default_action(const vki_siginfo_t *info, ThreadId tid)
{
   ...
   if (VG_(clo_trace_signals))
      ...
      VG_(message)(Vg_DebugMsg, "delivering %d (code %d) to default handler; action: %s%s",
		   sigNo, info->_info._code, terminate ? "terminate" : "ignore", core ? "+core" : "");
#else
      VG_(message)(Vg_DebugMsg, "delivering %d (code %d) to default handler; action: %s%s",
...
#if defined (VGO_netbsdelf2)
  if (VG_(clo_verbosity) > 1 || (could_core && info->_info._code > VKI_SI_USER)) { 
#else
	 ...
	 if (event != NULL) {
	    if (haveaddr)
	       ...
	       VG_(message)(Vg_UserMsg, " %s at address %p", 
			    event, info->_info._reason._fault._addr);
#else
	       VG_(message)(Vg_UserMsg, " %s at address %p", 
...
#if defined(VGO_netbsdelf2)
   info._info._signo = VKI_SIGSEGV;
   info._info._code = si_code;
   info._info._reason._fault._addr = (void*)addr;
#else
...
#if defined (VGO_netbsdelf2)
   info._info._signo = VKI_SIGILL;
   info._info._code = VKI_ILL_ILLOPC; /* jrs: no idea what this should be */
   info._info._reason._fault._addr = (void*)addr;
#else
...
void queue_signal(ThreadId tid, const vki_siginfo_t *si)
{
   ...
   */ #if defined (VGO_netbsdelf2)
   if (sq->sigs[sq->next]._info._signo != 0)
      VG_(message)(Vg_UserMsg, "Signal %d being dropped from thread %d's queue",
		   sq->sigs[sq->next]._info._signo, tid);
#else
   if (sq->sigs[sq->next].si_signo != 0)
	   VG_(message)(Vg_UserMsg, "Signal %d being dropped from thread %d's queue",sq->sigs[sq->next].si_signo, tid); 
#endif
...
#if defined(VGO_netbsdelf2)
   if (VG_(clo_trace_signals)) 
      VG_(message)(Vg_DebugMsg, "Async handler got signal %d for tid %d info %d",sigNo, tid, info->_info._code);
#else
   if (VG_(clo_trace_signals)) 
      VG_(message)(Vg_DebugMsg, "Async handler got signal %d for tid %d info %d",sigNo, tid, info->si_code);
#endif
...
#if defined(VGO_netbsdelf2)
   if (!is_sig_ign(info->_info._signo))
      deliver_signal(tid, info);
#else
...
*/
/* our siginfo structure is completely different from linux's so
...
/*  } */
	 ...
	 them manually. */
      if (info->_info._code == VKI_SI_TKILL)
...
/*    } */
   ...
   {   
	...
	resume_scheduler(tid);
      ...
      if (fault_catcher) {
	 (*fault_catcher)(sigNo, (Addr)info->_info._reason._fault._addr);
	 ...
	 Valgrind internally.
      ...
      VG_(message)(Vg_DebugMsg, 
		   ...
		   info->_info._code, info->_info._reason._fault._addr,
 ...
 */ void VG_(poll_signals)(ThreadId tid)
{
  ...
  static const struct vki_timespec zero = { 0, 0 };
   ...
   block_all_host_signals(&saved_mask); // protect signal queue /* try unblocking */
   ...
   if (sip == NULL && VG_(sigtimedwait)(&pollset, &si, &zero ) > 0) {
	 ...
	 VG_(message)(Vg_DebugMsg, "poll_signals: got signal %d "
                                   "for thread %d", si._info._signo, tid);
#else
	 VG_(message)(Vg_DebugMsg, "poll_signals: got signal %d "
   ...
   if (sip != NULL) {
	 ...
	 VG_(message)(Vg_DebugMsg, "Polling found signal %d for tid %d", 
		      sip->_info._signo, tid);
	 if (!is_sig_ign(sip->_info._signo))
		 deliver_signal(tid, sip);
	 else if (VG_(clo_trace_signals))
		 VG_(message)(Vg_DebugMsg, "signal %d ignored", sip->_info._signo);
	 sip->_info._signo = 0;	/* remove from signal queue, if that's
				   where it came from */ 
#else
      if (VG_(clo_trace_signals))
	 VG_(message)(Vg_DebugMsg, "Polling found signal %d for tid %d", 
...
/*    for (i = 1; i <= _VKI_NSIG; i++) { */
...
#ifndef VGO_netbsdelf2
	 tsa.sa_restorer = 0;
	 tsa.sa_flags = VKI_SA_SIGINFO;/* do we use all siginfo in netbsd? */ 
#endif
   ...
   scss.scss_per_sig[VG_SIGVGKILL].scss_flags   = VKI_SIG_IGN; /* VKI_SA_SIGINFO; */
